...
class WebpackPlugin {
    ...
    compiler.hooks.thisCompilation.tap('digital.ai.Protection', (compilation) => {
      compilation.hooks.processAssets.tapAsync(
        ...
        (assets, callback) => {
          ...
          if (targets.length > 1) {
            callback(new Error('Digital.ai Hybrid JavaScript Protection could not apply protection on multiple targets. Please remove all targets except one from the blueprint.'));
            return;
          }
  ...
  apply4(compiler) {
    compiler.hooks.emit.tapAsync('Arxan4Hybrid JavaScriptPlugin', (compilation, callback) => {
      const logger = compilation.getLogger('Arxan4Hybrid JavaScriptPlugin');
      ...
      const targets = Object.keys(blueprint[targetsKey]);
      if (targets.length > 1) {
        callback('Digital.ai Hybrid JavaScript Protection could not apply protection on multiple targets. Please remove all targets except one from the blueprint.');
        return;
      }
      let targetKey = targets[0];
