  ...
  return axios(options)
    .then((response) => {
      ...
      if (!filteredNames.length) {
        return Promise.reject(new Error(`Digital.ai Hybrid JavaScript Protection 7.5.0 could not be downloaded. Please contact ${SUPPORT_EMAIL} for help resolving this issue.`));
      }
      return { packageName: filteredNames[0].filename, accessToken };
    })
    .catch(() => Promise.reject(new Error('Digital.ai Hybrid JavaScript Protection 7.5.0 could not be downloaded. Make sure you have the "Product download" entitlement associated with the API key.')));
};
...
const getPackage = (accessToken, filename) => {
  ...
  return axios(options)
    ...
    .then(() => filename)
    .catch(error => Promise.reject(new Error(`Failed to download Digital.ai Hybrid JavaScript Protection 7.5.0. Error message: ${error.message}`)));
};
...
const extractDmg = filename => new Promise((resolve, reject) => {
  ...
  dmg.mount(downloadedFile, (mountError, mountPath) => {
    if (mountError) reject(new Error(`Failed to mount Digital.ai Hybrid JavaScript Protection 7.5.0 DMG package. ${mountError.message}`));
    fs.copySync(mountPath, configuration.installLocation);
    dmg.unmount(mountPath, (unmountError) => {
      if (unmountError) reject(new Error(`Failed to unmount Digital.ai Hybrid JavaScript Protection 7.5.0 DMG package. ${unmountError.message}`));
