...
const apiKey = process.env.A4HYBRID_API_KEY;
const apiSecret = process.env.A4HYBRID_API_SECRET;
if (!apiKey || !apiSecret) {
  exitWithMessage('Could not find environment variables required to download Arxan for Hybrid: A4HYBRID_API_KEY and A4HYBRID_API_SECRET. Refer to README to learn how to set it up correctly.');
}
  ...
  return request(options)
    .then((response) => {
      const filteredNames = response
        ...
        .filter(file => file.filename.startsWith('Arxan-for-Hybrid') || file.filename.startsWith('a4hybrid'));
      if (!filteredNames.length) {
        exitWithMessage('Arxan for Hybrid 3.7.0 could not be downloaded. Please contact support@arxan.com for help resolving this issue.');
      }
      return filteredNames[0].filename;
    })
    .catch(() => {
      exitWithMessage('Arxan for Hybrid 3.7.0 could not be downloaded. Make sure you have the "Product download" entitlement associated with the API key.');
    });
};
...
const getPackage = (accessToken, filename) => {
  ...
  return download(options.uri, configuration.installLocation, options)
    .catch((error) => {
      exitWithMessage(`Failed to download Arxan for Hybrid 3.7.0. Error message: ${error.message}`);
    });
};
...
const extractDmg = (filename) => {
  ...
  dmg.mount(downloadedFile, (mountError, mountPath) => {
    if (mountError) exitWithMessage(`Failed to mount Arxan for Hybrid 3.7.0 DMG package. ${mountError.message}`);
    fs.copySync(mountPath, configuration.installLocation);
    dmg.unmount(mountPath, (unmountError) => {
      if (unmountError) exitWithMessage(`Failed to unmount Arxan for Hybrid 3.7.0 DMG package. ${unmountError.message}`);
...
const writeMetadataFile = () => {
  ...
  fs.writeJson(metadataFile, jsonContents)
    .catch(() => {
      exitWithMessage('Internal error: Failed to write to metadata.json. Please contact support@arxan.com for help resolving this issue.');
    });
};
