...
const checkPackage = () => {
  ...
  if (metadata.version !== '6.3.0') {
    if (apiKey && apiSecret) {
      return downloadPackage();
    }
    return Promise.reject(new Error('Could not find environment variables required to download Digital.ai Hybrid JavaScript Protection: PROTECT_API_KEY and PROTECT_API_SECRET. Refer to README to learn how to set it up correctly.'));
  }
...
const fixUpBlueprint = blueprint => new Promise((resolve, reject) => {
  ...
  if (!existsSync(binaryLocation) && (!apiKey || !apiSecret)) {
    reject(new Error('Could not find environment variables required to download Digital.ai Hybrid JavaScript Protection: PROTECT_API_KEY and PROTECT_API_SECRET. Refer to README to learn how to set it up correctly.'));
  }
  ...
  reject(new Error('Could not find environment variable required to license Digital.ai Hybrid JavaScript Protection: PROTECT_LICENSE_TOKEN. Refer to README file to learn how to set it up.'));
});
